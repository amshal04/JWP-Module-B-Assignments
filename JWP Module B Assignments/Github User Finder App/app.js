...
const getUser = async () => {
  ...
  if (!username) {
    ...
    alert(`User "${username}" is already displayed.`);
    usernameInput.value = "";
    return;
  }
  ...
  userCard.innerHTML += `<div id="${loaderId}" class="spinner-border text-primary" role="status">
  ...
  return;
    ...
    // New card 
    userCard.innerHTML += `
    <div class="card text-center shadow position-relative" style="width: 22rem;" id="${username}">
    <button onclick="removeCard('${username}')" class="btn btn-sm btn-danger position-absolute top-0 end-0 m-2">❌</button>
        ...
        <div class="card-body">
          ...
          <a href="${data.html_url}" class="btn btn-primary mt-3" target="_blank">Visit Profile</a>
        </div>
      </div>
    `;
    
  } catch (error) {
    document.getElementById(loaderId).remove(); // Remove spinner 
    userCard.innerHTML += `<p class="text-danger">❌ ${error.message}</p>`;
  }
